...
```
FMP API (free)              Google News RSS (free)
  |                              |
  |-- Top gainers               |-- Headlines
  |-- Top losers                |
  |-- Most active               |
  |-- Company news              |
  |-- Earnings calendar         |
...
| **AI Agent** | The brain. Reads your message, decides which tool to call, and formulates the response. Uses the system prompt that defines its personality, rules, and risk disclosure requirements. |
...
| **scan_market** (tool) | Calls the Market Scanner workflow. Triggered when you mention a budget and want to scan. |
...
- The AI responds in whatever language you write in (default: Polish).
- Feedback tags (like "exited too early") are translated to your language when presented.
...
[Select Top 10 for News] -- pick top candidates for news lookup
    ...
    |
    v
[Merge News] -- combine news from both sources, deduplicate, limit 5 per ticker
...
| Node | What it does |
|---|---|
| **Trigger** | Receives parameters from the Chat AI Agent: budget (number), currency (USD/EUR/PLN), market (US/EU/BOTH). |
...
| **Filter Candidates** | Removes stocks that are unsuitable: penny stocks (< $1), stocks too expensive for the budget (> 30% of budget), OTC-listed stocks. Sorts by absolute price change and keeps top 10. |
...
| **Merge News** | Combines news from FMP and Google, groups by ticker, removes duplicates, limits to 5 per ticker. Resilient — uses try/catch on each source. |
...
| **Build AI Prompt** | The most important node. Assembles the complete prompt for AI analysis: all candidate data, news, earnings warnings, past insights, and detailed rules for signal quality and risk disclosure. Instructs AI to write reasoning in user's language. |
...
| **Validate & Enrich** | Parses the AI JSON response. Enforces hard rules that the AI might miss: minimum 60% confidence, minimum 1.5 risk/reward ratio. Calculates missing fields (shares, cost, profit/loss). Caps BUY signals at 5. Downgrades violations to SKIP with explanation. |
| **Format Response** | Converts validated signals into readable text that the Chat AI Agent will present to you. Includes all risk disclosure fields. |

**Resilience design:**

The scanner is built to survive failures. FMP free tier doesn't support all endpoints — Batch Quote and Earnings Check may return 403. Every external API node has "continue on failure" enabled, and every merge node uses try/catch. The pipeline always reaches AI Analysis with whatever data it could gather.
...
Key fields: ticker, type (BUY/SKIP), confidence, entry range, stop-loss, take-profit, suggested shares, estimated cost, potential profit/loss, risk/reward, risk level, risk warnings, reasoning, educational note, catalysts, price data snapshot, news data snapshot.

### trades

Your actual trading results. Linked to signals via `signal_id`.

Key fields: ticker, buy price, sell price, quantity, P&L (dollars and %), outcome (WIN/LOSS/BREAKEVEN), feedback tags, notes.
...
| **Yahoo Finance** | Fallback for quotes (not currently used, available for future) | Unlimited | Free |
...
*Blueprint version 3.1 — March 2026*
