...
Self-hosted via Docker (n8n + Postgres + SearXNG). Only external cost: OpenRouter API tokens (~$0.15-0.50/month).
...
└── External APIs:
    ...
    └── SearXNG (web search via Google/Bing/DDG) - self-hosted, free, no limits
```
...
```
trading-wizard/
├── docker-compose.yml          # n8n + Postgres + SearXNG orchestration
...
├── config/
│   └── searxng/
│       └── settings.yml        # SearXNG search engine config (JSON API enabled)
└── docs/
    ├── SETUP_GUIDE.md          # Step-by-step installation guide
    ├── HOW_IT_WORKS.md         # High-level system explanation
...
User message → 07-chat-main.json (AI Agent decides tool)
  ...
  └── web_search   → SearXNG (real-time web search)
```
...
- **Don't remove Finnhub integration** without explicit request - it provides supplementary news data.
...
- `YOUR_FINNHUB_KEY` - in Finnhub API URLs (query parameter `token=`)
...
| `docs/HOW_IT_WORKS.md`   | -     | High-level system explanation for non-technical readers                                     |
